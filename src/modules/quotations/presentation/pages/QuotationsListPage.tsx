    ...
    <div className="space-y-6">
      ...
      {loading ? <div className="rounded-xl border bg-white p-12 text-center text-sm text-slate-500">Carregando cotações reais...</div> : visible.length === 0 ? <div className="rounded-xl border bg-white p-12 text-center"><FileText className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-700">Nenhum registro nesta visão.</p><p className="mt-1 text-xs text-slate-500">A lista permanece vazia até existir um registro correspondente no Supabase.</p></div> : (
        <div className="overflow-hidden rounded-xl border bg-white shadow-sm">
          <table className="w-full text-left text-sm"><thead className="border-b bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="px-5 py-3">Código</th><th className="px-5 py-3">Tipo</th><th className="px-5 py-3">Solicitante</th><th className="px-5 py-3">Itens</th><th className="px-5 py-3">Data</th><th className="px-5 py-3">Status</th><th className="px-5 py-3 text-right">Ações</th></tr></thead><tbody className="divide-y">{visible.map(row => <tr key={row.id} className="hover:bg-indigo-50/40"><td className="px-5 py-4"><button onClick={() => navigate(`/quotations/${row.id}`)} className="font-bold text-indigo-700 hover:underline">{row.title}</button></td><td className="px-5 py-4">{row.request_type || 'Legado'}</td><td className="px-5 py-4">{row.requester_name_snapshot || 'Não registrado'}</td><td className="px-5 py-4">{row.quotation_items?.[0]?.count ?? 0}</td><td className="px-5 py-4">{new Date(row.created_at).toLocaleDateString('pt-BR')}</td><td className="px-5 py-4"><span className="rounded-full border px-2 py-1 text-[10px] font-bold">{STATUS_LABEL[row.status] || row.status}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => navigate(`/quotations/${row.id}`)} className="h-9 px-3 text-xs">Ver detalhes</Button><Button onClick={() => navigate(`/quotations/${row.id}/compare`)} className="h-9 bg-indigo-600 px-3 text-xs text-white">Comparar propostas</Button></div></td></tr>)}</tbody></table>
        </div>
      )}
    </div>
  );
