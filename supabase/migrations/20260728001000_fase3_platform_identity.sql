...
-- 9. BOOTSTRAP
-- Executa somente quando os registros necessários já existem.
-- Em bancos locais limpos, a migration estrutural não deve falhar.
...
BEGIN
    /*
     * A. Organização interna da plataforma
     */
    SELECT COUNT(*)
    INTO hub_org_count
    FROM public.organizations
    WHERE name ILIKE '%Hub.IA%';

    IF hub_org_count = 1 THEN
        SELECT id
        INTO hub_org_id
        FROM public.organizations
        WHERE name ILIKE '%Hub.IA%'
        ...
        WHERE id = hub_org_id
          AND is_platform_internal IS DISTINCT FROM true;

        IF FOUND THEN
            PERFORM public.insert_audit_log(
                'SET_PLATFORM_INTERNAL_ORGANIZATION',
                'organizations',
                hub_org_id,
                jsonb_build_object(
    ...
    ELSIF hub_org_count = 0 THEN
        RAISE NOTICE
            'Bootstrap ignorado: organização Hub.IA não encontrada.';

    ELSE
        RAISE EXCEPTION
            'Bootstrap abortado: foram encontradas % organizações correspondentes a Hub.IA.',
            hub_org_count;
    END IF;
    ...
    INTO users_count
    FROM auth.users
    WHERE lower(email) = lower('viniciuscordebello@gmail.com');
    ...
    IF users_count = 1 THEN
        ...
        PERFORM public.insert_audit_log(
            ...
            vini_user_id,
            jsonb_build_object(
                'email', 'viniciuscordebello@gmail.com',
                'source', 'migration_bootstrap'
            )
        );
    ...
    ELSIF users_count = 0 THEN
        RAISE NOTICE
            'Bootstrap ignorado: usuário viniciuscordebello@gmail.com não encontrado.';

    ELSE
        RAISE EXCEPTION
            'Bootstrap abortado: foram encontrados % usuários com o email viniciuscordebello@gmail.com.',
            users_count;
    END IF;
END;
$do$;