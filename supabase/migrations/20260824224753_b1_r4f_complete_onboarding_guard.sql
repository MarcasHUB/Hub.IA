...
BEGIN
  ...
  -- Validação CNPJ e Lock Advisory
  v_cnpj_normalized := regexp_replace(coalesce(p_org_document,''), '[^0-9]', '', 'g');
  IF length(v_cnpj_normalized) <> 14 THEN
      RAISE EXCEPTION 'ONBOARDING_REQUIRED_DATA_MISSING';
  END IF;

  PERFORM pg_advisory_xact_lock(hashtextextended(v_cnpj_normalized, 0));

  IF EXISTS (
    SELECT 1 FROM public.organizations 
    WHERE coalesce(cnpj_normalized, regexp_replace(coalesce(cnpj, ''), '[^0-9]', '', 'g')) = v_cnpj_normalized
  ) THEN
    RAISE EXCEPTION 'ONBOARDING_ORGANIZATION_ALREADY_EXISTS';
  END IF;
  ...
  IF array_length(v_resolved_segments, 1) > 0 THEN
    FOREACH v_seg_id IN ARRAY v_resolved_segments LOOP
      -- Escreve na nova fonte canônica (organization_segments)
      INSERT INTO public.organization_segments (organization_id, segment_id)
      VALUES (v_org_id, v_seg_id)
      ON CONFLICT DO NOTHING;

      -- Preserva o antigo para não quebrar dependentes legadas (temporário)
