-- 30_hotfix_f2_1_rpc_contract.sql

-- Remove a função antiga para permitir a mudança do tipo de retorno
DROP FUNCTION IF EXISTS public.validate_company_invite(text);

-- Cria a função correta
...
BEGIN
  ...
  -- 2. Calcula o hash (depende de extensions.digest da pgcrypto já instalada)
  v_token_hash := encode(extensions.digest(p_token, 'sha256'), 'hex');

  -- 3. Busca o convite pelo hash sem SELECT *
  SELECT
      i.id,
      i.email,
      i.company,
  ...
  -- 9. Dados convidados incompletos (strings vazias)
  ...
  -- 11. Busca a organização remetente (sem SELECT *)
  SELECT
      o.id,
      o.razao_social,
      o.nome_fantasia,
  ...
  -- 12. Organização ausente/incompleta
  IF NOT FOUND THEN
    validation_status := 'incomplete';
    RETURN NEXT;
    RETURN;
  ...
  -- 13. Popula os dados SOMENTE NO ESTADO VALID
...
-- Restaura explicitamente os grants para a role de anon (o fluxo de onboarding começa deslogado)
