...
async function onRequestPost(context) {
  ...
  try {
    let joinUrl2 = function(baseUrl, path) {
      const cleanBase = baseUrl.replace(/\/+$/, "");
      const cleanPath = path.replace(/^\/+/, "");
      return `${cleanBase}/${cleanPath}`;
    }, formatCNPJ2 = function(value) {
      if (!value) return "";
      const digits = value.replace(/\D/g, "");
      if (digits.length !== 14) return value;
      return digits.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, "$1.$2.$3/$4-$5");
    };
    var joinUrl = joinUrl2, formatCNPJ = formatCNPJ2;
    ...
    if (action === "operator_invite") {
      const { data: invite, error: fetchError } = await supabase.from("operator_invitations").select("*").eq("token", target_id).single();
      if (fetchError || !invite) {
        return new Response(JSON.stringify({ error: "Convite n\xE3o encontrado ou RLS bloqueou o acesso" }), {
          status: 404,
          headers: { "Content-Type": "application/json", ...corsHeaders }
        });
      }
      ...
      const link = joinUrl2(publicUrl, `aceitar-convite?token=${invite.token}`);
      const manualUrl = joinUrl2(publicUrl, "aceitar-convite");
      ...
      if (c.includes("[APP] Solicitante")) {
        ...
        mobile = true;
        desktop = false;
        perms = ["Criar solicita\xE7\xF5es.", "Consultar andamento das solicita\xE7\xF5es.", "Acompanhar aprova\xE7\xF5es."];
        rests = ["N\xE3o aprova solicita\xE7\xF5es.", "N\xE3o realiza compras.", "N\xE3o acessa m\xF3dulos administrativos."];
      } else if (c.includes("[DESKTOP] Auditor")) {
        roleName = "Auditor";
        mobile = false;
        desktop = true;
        perms = ["Consultar processos.", "Consultar hist\xF3ricos.", "Consultar aprova\xE7\xF5es.", "Consulta de processos encerrados."];
        rests = ["N\xE3o aprova.", "N\xE3o compra.", "N\xE3o cria solicita\xE7\xF5es.", "N\xE3o altera registros."];
      } else if (c.includes("[DESKTOP] Gestor") || invite.perfil === "gestor") {
        roleName = "Gestor";
        perms = ["Aprovar solicita\xE7\xF5es.", "Aprovar valores conforme pol\xEDtica.", "Acompanhar solicita\xE7\xF5es da equipe.", "Delegar aprova\xE7\xF5es quando permitido."];
      } else if (c.includes("[DESKTOP] Administrador") || invite.perfil === "administrador") {
        roleName = "Administrador";
        perms = ["Administra\xE7\xE3o do sistema.", "Gest\xE3o de usu\xE1rios.", "Configura\xE7\xF5es gerais.", "Gest\xE3o operacional completa."];
      } else {
        roleName = "Comprador";
        perms = ["Receber solicita\xE7\xF5es.", "Realizar cota\xE7\xF5es.", "Comparar fornecedores.", "Emitir or\xE7amentos.", "Conduzir processos de compra.", "Acompanhar negocia\xE7\xF5es."];
      }
      let categoryList = [];
      if (invite.todas_categorias) {
        categoryList = ["Todas as Categorias"];
      } else if (invite.category_ids && invite.category_ids.length > 0) {
        const { data: cats } = await supabase.from("categories").select("name").in("id", invite.category_ids);
        if (cats) categoryList = cats.map((c2) => c2.name);
      }
      ...
      const restsHTML = rests.length > 0 ? `
        <h4 style="margin-top: 16px; margin-bottom: 0; color: #d32f2f;">Restri\xE7\xF5es</h4>
      ...
      const categoriesHTML = categoryList.length > 0 ? `
        ...
        <ul style="padding-left: 20px; margin-top: 4px; color: #444;">
          ${categoryList.map((c2) => `<li>${c2}</li>`).join("")}
        </ul>
      ` : "";
      const infoBlockHTML = `
        <div style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-top: 24px; margin-bottom: 24px;">
          ...
          <h4 style="margin-top: 16px; margin-bottom: 0; color: #374151;">Acessos Dispon\xEDveis</h4>
          ${accessChannelsHTML}
          
          <h4 style="margin-top: 16px; margin-bottom: 0; color: #374151;">Permiss\xF5es</h4>
          ${permsHTML}
...
${categoryList.map((c2) => `- ${c2}`).join("\n")}` : ""}
`;
      const welcomeText = "Voc\xEA foi convidado para acessar a plataforma.";
      htmlContent = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
          <h2>Bem-vindo \xE0 SupplyHUB!</h2>
          <p>Ol\xE1 <strong>${toName}</strong>,</p>
          <p>${welcomeText}</p>
          
          ${infoBlockHTML}

          <p>Clique no bot\xE3o abaixo para concluir seu cadastro:</p>
          <p>
          ...
          <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; font-size: 14px; color: #666;">
            <p>Se o bot\xE3o n\xE3o abrir no celular, acesse o link abaixo no navegador:</p>
            ...
            <p>E cole o c\xF3digo do convite abaixo.</p>
            
            <p style="margin-bottom: 4px; font-weight: bold; color: #333;">C\xF3digo do convite:</p>
      ...
      `;
      textContent = `Ol\xE1 ${toName},

${welcomeText}
${infoBlockText}
Clique no bot\xE3o ou copie e cole este link no navegador para concluir seu cadastro:
${link}

C\xF3digo do convite:
${invite.token}

Este convite expira em 72 horas.`;
    }
    if (action === "supplier_invite") {
      const { data: invite, error: fetchError } = await supabase.from("invitations").select("*").eq("token_hash", target_id).single();
      if (fetchError || !invite) {
        return new Response(JSON.stringify({ error: "Convite de empresa n\xE3o encontrado ou RLS bloqueou o acesso" }), {
          status: 404,
          headers: { "Content-Type": "application/json", ...corsHeaders }
        });
      }
      ...
      const link = joinUrl2(publicUrl, `onboarding?token=${invite.token_hash}`);
      const inviterName = user?.user_metadata?.nome ? `${user.user_metadata.nome} ${user.user_metadata.sobrenome || ""}`.trim() : "Usu\xE1rio do Sistema";
      ...
      htmlContent = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333; line-height: 1.5;">
          <h2 style="color: #111827;">Bem-vindo \xE0 Rede Hub.IA</h2>
          <p>Ol\xE1 <strong>${toName}</strong>,</p>
          <p>Sua empresa foi convidada para participar da <strong>Rede Hub.IA de Suprimentos</strong>.</p>
          <p>A Hub.IA conecta empresas compradoras e fornecedoras em um ambiente \xFAnico para gera\xE7\xE3o de neg\xF3cios, cota\xE7\xF5es, parcerias estrat\xE9gicas e oportunidades comerciais.</p>
          ...
          <div style="background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin: 24px 0;">
            ...
            <p style="margin: 8px 0 0 0;"><strong>Raz\xE3o Social:</strong> ${invite.company}</p>
            <p style="margin: 4px 0 0 0;"><strong>CNPJ:</strong> ${formatCNPJ2(invite.document)}</p>
            ${invite.city && invite.state ? `<p style="margin: 4px 0 0 0;"><strong>Localiza\xE7\xE3o:</strong> ${invite.city}/${invite.state}</p>` : ""}
          </div>

          <p style="font-weight: bold; margin-bottom: 8px;">Pr\xF3ximo passo</p>
          ...
          <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #666;">
            <p>Se o bot\xE3o n\xE3o funcionar, copie e cole o link abaixo no seu navegador:</p>
            <p style="word-break: break-all;"><a href="${link}" style="color: #4F46E5;">${link}</a></p>
...
Bem-vindo \xE0 Rede Hub.IA

Sua empresa foi convidada para participar da Rede Hub.IA de Suprimentos.
A Hub.IA conecta empresas compradoras e fornecedoras em um ambiente \xFAnico para gera\xE7\xE3o de neg\xF3cios, cota\xE7\xF5es, parcerias estrat\xE9gicas e oportunidades comerciais.

Dados da Empresa:
Raz\xE3o Social: ${invite.company}
CNPJ: ${formatCNPJ2(invite.document)}
